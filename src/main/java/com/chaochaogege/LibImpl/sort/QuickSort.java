...
public class QuickSort {
    ...
    public QuickSort SortV2(int arr[], int head, int tail) {
        if (head > tail ){
            return this;
        }
        int pivot = arr[head];
        int srcHead = head, srcTail = tail;
        while(head < tail ) {
            while(head < tail && arr[tail] >= pivot) {
                tail --;
            }
